<?php
/**
 * Gadgets. Apps that extend the core functionality of Jaws are called gadgets. Gadgets can do just about anything that's possible on the web.
 *
 * @category   Gadget
 * @category   feature
 * @package    Core
...
class Jaws_Gadget
{
    ...
    function Init($gadget)
    {
        ...
        if (Jaws_Error::IsError($info)) {
            Jaws_Error::Fatal("Gadget $gadget needs Info File!", __FILE__, __LINE__);
        }
     ...
     */
    function AddAction($action, $mode, $description)
    {
    ...
    function ValidateAction()
    {
        ...
        if (!($this->IsValidAction($action))) {
            Jaws_Error::Fatal('Invalid action: '.$action, __FILE__, __LINE__);
            return false;
        }
     ...
     * @return  boolean True if action is valid, if not, returns false
     */
                ...
                foreach ($plugins as $plugin) {
                    ...
                    if (file_exists($file)) {
                        ...
                        if (!Jaws_Error::isError($use_in) && ($use_in == '*' || in_array($gadget, explode(',', $use_in)))) {
                            if (!isset($GLOBALS['plugins'][$plugin]['obj'])) {
                                require_once $file;
                                $GLOBALS['plugins'][$plugin]['obj'] = new $plugin();
                                $res = $GLOBALS['plugins'][$plugin]['obj']->ParseText($res);
                            } else {
                                $res = $GLOBALS['plugins'][$plugin]['obj']->ParseText($res);
                            }
                        }
    ...
    function _commonDisableGadget($gadget)
    {
        ...
        $gadgets = $GLOBALS['app']->Registry->Get('/gadgets/allowurl_items');
        if (stristr($gadgets, $gadget)) {
            $gadgets = str_replace(','.$gadget, '', $gadgets);
        }
        $GLOBALS['app']->Registry->Set('/gadgets/allowurl_items', $gadgets);
    ...
    function UpdateGadget($gadget)
    {
        ...
        $data    = $GLOBALS['app']->Registry->Get('/gadgets/allowurl_items');
        $gadgets = explode(',', $data);
        if ($info->AllowListURL()) {
            if (!in_array($gadget, $gadgets)) {
                $data .= ',' . $gadget;
                $GLOBALS['app']->Registry->Set('/gadgets/allowurl_items', $data);
            }
    ...
    function EnableGadget($gadget)
    {
        if (strtolower($gadget) === 'core') {
            return new Jaws_Error(_t('GLOBAL_GADGETS_GADGET_CANT_HAVE_NAME_CORE', $gadget), $gadget);
        }
        ...
        if (is_array($req)) {
            foreach ($req as $r) {
                if (!Jaws_Gadget::IsGadgetInstalled($r)) {
                    return new Jaws_Error(_t('GLOBAL_GI_GADGET_REQUIRES', $r, $gadget), $gadget);
                }
            }
        }
    ...
    {
        return $GLOBALS['app']->Session->CheckPermission(empty($gadget)? $this->_Name : $gadget, $task, $errorMessage);
    }
