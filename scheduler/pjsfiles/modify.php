...
if ($GLOBALS['app']->Session->Logged()) {
	...
	if (ereg("minute",$interval_array[1])>0) $minutes=$interval_array[0];
	else $minutes=-1;
	if (ereg("hour",$interval_array[1])>0) $hours=$interval_array[0];
	else $hours=-1;
	if (ereg("day",$interval_array[1])>0) $days=$interval_array[0];
	else $days=-1;
	if (ereg("week",$interval_array[1])>0) $weeks=$interval_array[0];
	else $weeks=-1;
	include("header.html");
