...
class Rounded_Rectangle
{
	 ...
	 * @return	void
	 */
	function Rounded_Rectangle($params)
	{
		$this->image = '';						# image resource
		$this->width = 100;				# width of rectangle
		$this->height = 100;				# height of rectangle
		$this->radius = 10;				# radius of corner
		$this->foreground = 'CCC';		# color of corner
		$this->background = 'FFF';		# color of background
		$this->borderwidth = 0;			# width of border
		$this->bordercolor = '000';		# color of border (if border width > 0)
		$this->bgtransparent = false;		# transparent background flag
		$this->btransparent = false;		# transparent border flag
		$this->fgtransparent = false;		# transparent foreground flag
		$this->antialias = true;			# antialias flag
		$this->tail = true;			# has a tail flag
	...
	{
		if ($this->tail) {
			$this->image = imagecreatetruecolor($this->width, $this->height+($this->radius-$this->borderwidth));
		} else {
		...
		if ($this->tail) {
			$img2 = Rounded_Corner::create(array('radius' => $this->radius*2, 
							'orientation' => 'tl', 
							'foreground' => $this->foreground, 
							...
							'fgtransparent' => true,
							'antialias' => $this->antialias));
			imagecopy($this->image, $img2, (-3)-$this->borderwidth, $this->height-$this->borderwidth, 0, 0, $this->radius*2, $this->radius);
			$img3 = $this->create(array('radius' => 0,
				'width'			=> $this->width,
				'height'		=> $this->height,
				'foreground'	=> $this->foreground,
				'background'	=> $this->background,
				...
				'fgtransparent'	=> true,
				'tail'			=> false));
			imagecopy($this->image, $img3, $this->radius*2+((-3)-($this->borderwidth)), $this->height, 0, 0, $this->width, $this->height);
