	...
	return Plugin.create('image', {
		...
		defaultSettings: {
			...
			onRemove: function ($image) {
				Aloha.Log.info('Default onRemove invoked', $image);
				return false;
			},
		...
		/**
		 * Internal callback hook which gets invoked when removing images
		 */
		_onRemove: function ($image) {
			
			$image.remove();
			
			// No default behaviour defined besides event triggering
			$('body').trigger('aloha-image-remove', $image);
			
			FloatingMenu.setScope('Aloha.global');
			
			// Call the custom resize function
			return this.onRemove($image);
		},
		...
		*/
		initializeButtons: function() {
			...
			if (this.settings.ui.remove) {
				var tabId = this.settings.ui.remove ? tabImage : tabImage;
				that._addUIRemoveButton(tabId);
			}
		...
		/**
		 * Adds the remove button to the floating menu for the given tab 
		 */
		_addUIRemoveButton: function(tabId) {
			var that = this;
			// remove button
		...
		_addUIInsertButton: function(tabId) {
			...
			this.insertImgButton = new Aloha.ui.Button({
				...
				'size' : 'small',
				'onclick' : function () { insertFile('', 'Insert File', that.imgSrcField.id); },
		...
		bindInteractions: function () {
			...
			if (this.settings.ui.meta) {
				...
				this.imgSrcField.addListener('change', function(obj, event) {
					that.srcChange();
				});
			...
			// Override the default method by using the given one
			if (this.settings.onRemove && typeof this.settings.onRemove === "function") {
				this.onRemove = this.settings.onRemove;
			}
			...
			try {
				if ( Aloha.activeEditable ) {
					...
					if (targetObj.hasClass('Aloha_Image_Resize')) {
						alert((targetObj.hasClass('Aloha_Image_Resize')));
					} else {
						alert('typeof range.startContainer.childNodes: ' + range.startContainer.childNodes.length);
						alert('typeof range.startOffset: ' + range.startOffset);
						alert('typeof range.startContainer.childNodes[range.startOffset]: ' + typeof range.startContainer.childNodes[range.startOffset]);
						alert('range.startContainer.childNodes[range.startOffset].nodeName.toLowerCase(): ' + range.startContainer.childNodes[range.startOffset].nodeName.toLowerCase());
						alert(range.startOffset+1 + ' === ' + range.endOffset);
					}
					*/
						...
						(targetObj.hasClass('Aloha_Image_Resize')))
					{
						...
						Aloha.Log.info('Cannot find img markup.', that.name);
						return null;
					}
				} else {
					Aloha.Log.info('Didn\'t look for img markup, because current editable is not active.', that.name);
				}
			} catch (e) {
		...
		*/		
		insertImg: function(imgSrc) {
			...
			if (typeof(imgSrc) == "undefined") {
				imgSrc = imagePluginUrl + '/img/blank.jpg';
			}
			...
			if ( range.isCollapsed() ) {
				...
				imagetag = '<img style="'+ imagestyle + '" src="' +imgSrc + '" title="" />';
...
//			 this.imgSrcField.getItem(); // (optinal a selected resource item)
		...
		remove: function() {
			...
			if (this._onRemove(this.imageObj)) {
				// the external reset procedure has already performed a reset, so there is no need to apply an internal reset
				return;
			}
