...
function PagePlayer(oConfigOverride) {
  ...
  this.config = {
    ...
    repeat: false,         	// repeat or stop at end
  ...
  this.events = {
    ...
    play: function() {
	  ...
	  if (!checkCookie()) {
		...
		sm.pauseAll();
		/*
		var o = self.getTheDamnTarget({target:pl.links[0]});
		...
		*/
		//pl.handleClick({target:pl.links[0]}); // fake a click event - aren't we sneaky. ;)
	  }
    },
    ...
    finish: function() {
      ...
      if (self.config.playNext && this._data.nIndex<pl.links.length-1 && self.config.repeat === false) {
        pl.handleClick({target:pl.links[this._data.nIndex+1]}); // fake a click event - aren't we sneaky. ;)
      } else if (self.config.playNext && self.config.repeat === true) {
		var o = self.getTheDamnTarget({target:pl.links[0]});
