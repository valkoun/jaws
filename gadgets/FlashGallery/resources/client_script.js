    ...
    savequickadd: function(response) {
		...
		if (response['success']) {
			if (response['addtype'] == 'Scrumy') {
				...
				window.top.saveUpdate(response['id'], comment_html, '', 0, response['sharing'], parent.$('syndication').checked, (parent.$('OwnerID') ? parent.$('OwnerID').value : ''), 'FlashGallery', true, false, response['eaurl'], false);
			} else {
				//$('layout_main').appendChild(document.createTextNode(response['elementbox']));
				// Fragile!, it must be equal to admin_CustomPage_view template
				parent.selectGadget('FlashGallery', 'AddGadget', '', parent.prevLinkID, parent.prevSectionID);
				return true;
				...
				*/
				...
				var tableDnD = new window.top.GalleryTableDnD();
				tableDnD.init(tbl);             
				//items['main']['item_' + response['id']] = true; 
				//newdrags[response['id']] = new Draggable('item_' + response['id'], {revert:true,constraint:true});
			}
...
function addFileToPost(gadget, table, method, syntactsCategory, linkid, num, width, height)
{
	...
	for (n=0; n<num; n++) {
		...
		if (num > 1) {
			ifrm.setAttribute("src", "index.php?gadget=FileBrowser&action=account_AddFileToPost&linkid="+linkid+"&table="+table+"&method=" + method + "&addtogadget=" + gadget + "&n=" + (fileCount+1) + "&bc=" + dItem.style.backgroundColor);
		} else {
			ifrm.setAttribute("src", "index.php?gadget=FileBrowser&action=account_AddFileToPost&where="+linkid+"&table="+table+"&method=" + method + "&addtogadget=" + gadget + "&n=" + (fileCount+1) + "&bc=" + dItem.style.backgroundColor);
		}
...
var galleriesAsync = new flashgalleryajax(FlashGalleryCallback);
//galleriesAsync.serverErrorFunc = Jaws_Ajax_ServerError;

var galleriesSync = new flashgalleryajax();
//galleriesSync.serverErrorFunc = Jaws_Ajax_ServerError;
