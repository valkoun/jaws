    ...
    savequickadd: function(response) {
		...
		if (response['success']) {
			if (response['addtype'] == 'Scrumy') {
				...
				window.top.customPageSelectGadget('Properties', 'AddGadget', '', window.top.prevLinkID, window.top.prevSectionID);
				return true;
...
function getClosestMatch(value, pid, element, autocomplete, table)
{
    ...
    var match = propertiesSync.getclosestmatch(value, pid, table);
...
function saveSettings()
{
	...
	if ($('city').value != "") {
		old_city = $('city').value;
		getClosestMatch($('city').value, $('region').value, 'city', 'city_choices', 'country');
    ...
    var shipfrom_city     = $('city').value;
    var shipfrom_state     = $('region').value;
    var shipfrom_zip     = $('postal_code').value;
	*/
	var showmap 				= $('showmap').value;
	var showcalendar			= $('showcalendar').value;
	var randomize 				= $('randomize').value;
    var user_post_limit  	 	= $('user_post_limit').value;
    var user_desc_char_limit   	= $('user_desc_char_limit').value;
    var user_mask_owner_email   = $('user_mask_owner_email').value;
    var user_min_price     		= $('user_min_price').value+'';
    var user_max_price     		= $('user_max_price').value+'';
    var user_status_limit 		= new Object();
    list = document.getElementsByName('user_status_limit[]')
    var j = 0;
	for (var i=0;i<list.length;i++){
		if(list[i].checked){
	    	user_status_limit[j] = list[i].value;
			j++;
		}
    }
        
    properties.savesettings(showmap, user_post_limit, user_desc_char_limit, user_mask_owner_email, user_min_price, user_max_price, user_status_limit, randomize, showcalendar);
}
...
var properties = new propertiesadminajax(PropertiesCallback);

var propertiesSync = new propertiesadminajax();
