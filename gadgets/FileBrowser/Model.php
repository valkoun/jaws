...
class FileBrowserModel extends Jaws_Model
{
     ...
     */
    function GetCurrentRootDir($current_dir, $root_dir = null)
    {
        if (is_null($root_dir)) {
			$root_dir = $this->GetFileBrowserRootDir();
		}
		if (!is_dir($root_dir . $current_dir)) {
    ...
    function DBFileInfo($path, $file)
    {
        ...
        $sql = 'SELECT
                    [id], [path], [filename], [title], [description], [fast_url]
                FROM [[filebrowser]]
                WHERE
                    [path]     = {path}
                  AND
     ...
     * @return  array   A list of properties of file
     */
    function GetFileProperties($path, $fname, $root_dir = null)
    {
        if (is_null($root_dir)) {
			$root_dir = $this->GetFileBrowserRootDir();
		}
        ...
        $url = $GLOBALS['app']->getDataURL(str_replace('//', '/', str_replace(JAWS_DATA, '', $root_dir).'/'.$path.'/'.$fname), true);
        $file['url'] = $url;
		require_once JAWS_PATH . 'include/Jaws/Image.php';
		$thumb = Jaws_Image::GetThumbPath(str_replace('//', '/', str_replace(JAWS_DATA, '', $root_dir).'/'.$path.'/'.$fname));
		$medium = Jaws_Image::GetMediumPath(str_replace('//', '/', str_replace(JAWS_DATA, '', $root_dir).'/'.$path.'/'.$fname));
		if (file_exists(JAWS_DATA . $thumb)) {
			$file['thumb'] = $GLOBALS['app']->getDataURL($thumb, true);
		}
		if (file_exists(JAWS_DATA . $medium)) {
			$file['medium'] = $GLOBALS['app']->getDataURL($medium, true);
		}
        ...
        if (is_file($image_path . $iconName)) {
            ...
            if (!is_file(JAWS_PATH . 'gadgets/FileBrowser/images/'.$iconName)) {
                $icon =  $GLOBALS['app']->GetJawsURL() . '/gadgets/FileBrowser/images/mimetypes/text-x-generic.png';
            } else {
        ...
        $file['ext'] = $ext;
        //Fullpath
        $filepath = $root_dir . $path . $fname;
        $file['fullpath'] = $filepath;
        //Set the icon
        $file['mini_icon'] = $GLOBALS['app']->GetJawsURL() . '/gadgets/FileBrowser/images/mini_file.png';
     ...
     */
    function GetDirProperties($path, $dirname, $root_dir = null)
    {
        if (is_null($root_dir)) {
			$root_dir = $this->GetFileBrowserRootDir();
		}
        ...
        //Fullpath
        $filepath = $root_dir . $path . $dirname;
        ...
        //hold.. is it shared?
        if ($dir['is_shared']) {
            $dir['icon'] = $GLOBALS['app']->GetJawsURL() . '/gadgets/FileBrowser/images/folder-remote.png';
            $dir['mini_icon'] =  $GLOBALS['app']->GetJawsURL() . '/gadgets/FileBrowser/images/folder-remote.png';
        } else {
            $dir['icon'] = $GLOBALS['app']->GetJawsURL() . '/gadgets/FileBrowser/images/folder.png';
            $dir['mini_icon'] = $GLOBALS['app']->GetJawsURL() . '/gadgets/FileBrowser/images/mini_folder.png';
        }
     ...
     * @return  array   A list of properties of files and directories of a certain path and Jaws_Error on failure
     */
    function ReadDir($path, $limit = 0, $offset = 0, $root_dir = null)
    {
        if (is_null($root_dir)) {
			$root_dir = $this->GetFileBrowserRootDir();
		}
		...
		foreach ($adr as $file) {
            ...
            if ($file{0} != '.') {
                $file_counter ++;
                $filepath = $root_dir . $path . $file;
				if (is_dir($filepath)) {
                    $files[$file_counter] = $this->GetDirProperties($path, $file, $root_dir);
                } else {
                    $files[$file_counter] = $this->GetFileProperties($path, $file, $root_dir);
                }
            }
        }
     ...
     */
    function GetDirContentsCount($path, $root_dir = null)
    {
        if (is_null($root_dir)) {
			$root_dir = $this->GetFileBrowserRootDir();
		}
