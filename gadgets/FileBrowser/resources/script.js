...
function saveFile()
{
    if (jawsTrim($('uploadfile').value) == '' && jawsTrim($('filename').value) == '') {
        alert(incompleteFields);
        return false;
    }
    if ($('upload_switch').checked && jawsTrim($('uploadfile').value) != '') {
...
{
    var insert = (datagrid_name == 'fp_datagrid' ? true : false);
	var result = filebrowserSync.getdirectory($('path').value, offset, insert);
    if (reset) {
