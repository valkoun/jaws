<?php
/**
 * CustomPage - Subscribe gadget hook
 *
 * @category   GadgetHook
 * @package    CustomPage
...
{			
    ...
    function CurrentURLSubscriptions($request_gadget = null, $request_action = null, $request_id = null)
    {
		...
		if ($request_gadget == 'CustomPage' && $request_action == 'Page' && !is_null($request_id)) {
			$pageModel = $GLOBALS['app']->LoadGadget('CustomPage', 'Model');
			$page = $pageModel->GetPage($request_id);
			if (!Jaws_Error::IsError($page) && isset($page['linkid']) && !empty($page['linkid']) && $page['gadget'] == 'Users') {
				if ($page['gadget_action'] == 'GroupPage') {
					$groupInfo = $jUser->GetGroupInfoById($page['linkid']);
				} else if ($page['gadget_action'] == 'UserPage') {
					$userInfo = $jUser->GetUserInfoById($page['linkid']);
				}
			}
		}
