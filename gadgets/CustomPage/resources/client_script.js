...
var CustomPageCallback = { 
		...
		if (response['success']) {
			if (response['addtype'] == 'Scrumy') {
				...
				if (response['laction'] && response['laction'] != '') {
					...
					customPageExecuteFunctionByName('pages.addgadget', window, response['lgadget'], response['laction'], response['page_gadget'], response['page_action'], response['page_linkid'], response['items_on_layout']);
				}
			}
		}
	},
	...
	addgadget: function(response) {
        if (response['success']) {
            ...
            window.top.$(response['section_name']).innerHTML = window.top.$('custom_page-main').innerHTML + response['gadget_html'];
...
function customPageAddGadgetToLayout(gadget, action)
{   
	//window.top.showWorkingNotification();
    // Ugly hack to add gadget from the greybox
	// TODO: Maybe pass items_on_layout as Object, so it's not a crazy long string...
	fun = 'pages.addgadget(\'' + gadget + '\',\'' + action + '\', ($(\'page_gadget\') ? $(\'page_gadget\').value : null), ($(\'page_action\') ? $(\'page_action\').value : null), ($(\'page_linkid\') ? $(\'page_linkid\').value : null), window.top.items_on_layout)';
	setTimeout(fun, 0);
	var wm = window.top.UI.defaultWM;
    var windows = wm.windows();
    windows.first().destroy();
...
function customPageGetQuickAddForm(g, method, id, linkid, callback)
{
	...
	if ($('quick_add_'+g)) {
		...
		customPageIfrm[g].setAttribute("src", "index.php?gadget="+g+"&action=account_GetQuickAddForm&method="+method+"&id="+id+"&linkid="+linkid);
		customPageIfrm[g].style.width = "100%";
		customPageIfrm[g].style.height = "5000px";
...
function customPageSelectGadget(g, method, id, linkid, callback)
{
    $('gadget').value = g;
	...
	if (g == 'Text') {
		...
		var actions = pagesSync.getgadgetactions(g);
		var first = null;
		show_link = false;
		link_shown = false; 
...
function customPageAddFileToPost(gadget, table, method, syntactsCategory, linkid, customPageNum, width, height, bgc, focus, base_url)
{
	...
	for (n=0; n<customPageNum; n++) {
		...
		*/
		...
		if (customPageNum > 1) {
			customPageIfrm.setAttribute("src", base_url + "index.php?gadget=FileBrowser&action=account_AddFileToPost&linkid="+linkid+"&table="+table+"&method=" + method + "&addtogadget=" + gadget + "&n=" + (customPageFileCount+1) + "&bc=" + dItem.style.backgroundColor);
		} else {
			customPageIfrm.setAttribute("src", base_url + "index.php?gadget=FileBrowser&action=account_AddFileToPost&where="+linkid+"&table="+table+"&method=" + method + "&addtogadget=" + gadget + "&n=" + (customPageFileCount+1) + "&bc=" + dItem.style.backgroundColor);
		}
...
function setTemplatesOnSelect()
{
	...
	for(var i=0; i<length; i++) {
		...
		if (className == realName) {
			if ($('theme') && cboxes[i].value != '' && cboxes[i].value == $('theme').value) {
 ...
 * Save EditPost
 */
function saveEditPost(id, content, addtype, method)
{
	if (content != prevContent) {
		...
		*/
		var response = pagesSync.saveeditpost(id, content, addtype, method);
		showResponse(response);
	}
}
...
var pages = new custompageajax(CustomPageCallback);
//pages.serverErrorFunc = Jaws_Ajax_ServerError;
var pagesSync = new custompageajax();
