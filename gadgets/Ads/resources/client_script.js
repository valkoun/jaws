...
var AdsCallback = { 
    ...
    deletesavedad: function(response) {
        if (response[0]['css'] == 'notice-message') {
            getSavedAdsData();
        }
		hideWorkingNotification();
    ...
    massivedeletesavedads: function(response) {
        if (response[0]['css'] == 'notice-message') {
            var rows = $('savedads_datagrid').getSelectedRows();
            if (rows.length > 0) {
                for(var i=0; i<rows.length; i++) {
                    $('savedads_datagrid').deleteItem();
                }
            }
            PiwiGrid.multiSelect($('savedads_datagrid'));
            getSavedAdsData();
        }
        showResponse(response);      
    },
...
/**
 * Adds a click record to db table : function
 */
function addClick(id, url)
{
    //selectedCalendar = cid;
	currentAction = 'AddClick';
	var response = adsSync.addclick(id);
	if (response[0]['css'] == 'notice-message') {
		window.open(url);
		//stopAction();
   }
 ...
 * Can use massive delete?
 */
function massiveDeleteSavedAds(message) 
{
    var rows = $('savedads_datagrid').getSelectedRows();
    if (rows.length > 0) {
        var confirmation = confirm(message);
        if (confirmation) {
            adsAsync.massivedeletesavedads(rows);
        }
    }
}
 ...
 * Search for pages and translations
 */
function searchSavedAds()
{
    updateAdsDatagrid($('savedads_status').value, $('savedads_search').value, 0, true);
}
 ...
 * Get pages data
 */
function getSavedAdsData(limit)
{
    if (limit == undefined) {
        limit = $('savedads_datagrid').getCurrentPage();
    }
    updateSavedAdsDatagrid($('savedads_status').value,
                        $('savedads_search').value,
                        limit,
                        false);
}
 ...
 * Get previous values of pages
 */
function previousSavedAdsValues()
{
    var previousSavedAdsValues = $('savedads_datagrid').getPreviousPagerValues();
    getSavedAdsData(previousSavedAdsValues);
    $('savedads_datagrid').previousPage();
}

/**
 * Get next values of pages
 */
function nextSavedAdsValues()
{
    var nextSavedAdsValues = $('savedads_datagrid').getNextPagerValues();
    getSavedAdsData(nextSavedAdsValues);
    $('savedads_datagrid').nextPage();
}

/**
 * Update pages datagrid
 */
function updateSavedAdsDatagrid(status, search, limit, resetCounter)
{
	showWorkingNotification();
    $('savedads_datagrid').objectName = adsSync;
    JawsDataGrid.name = 'savedads_datagrid';

    var result = adsSync.searchsavedads(status, search, limit);
    resetGrid('savedads_datagrid', result);
    if (resetCounter) {
        var size = adsSync.sizeofsearch3(status, search);
        $('savedads_datagrid').rowsSize    = size;
        //$('pages_datagrid').setCurrentPage(0);
        $('savedads_datagrid').updatePageCounter();
    }
	hideWorkingNotification();
}
...
var adsAsync = new adsajax(AdsCallback);
/*
