 ...
 */
class AdsHTML extends Jaws_GadgetHTML
{
    var $_Name = 'Ads';
    /**
     * Constructor
     ...
     *
     * @param 	int 	$id 	Ad ID (optional)
     * @param 	boolean 	$embedded 	Embedded mode
     * @param 	string 	$referer 	Embedding referer
     * @access 	public
     * @return 	string 	HTML template
     */
    function Ad($id = null, $embedded = false, $referer = null)
    {
		require_once JAWS_PATH . 'include/Jaws/Image.php';
    ...
    /**
     * Displays an XML file with the requested ads
     *
     * @access public
     * @return string
     */
    function AdsXML()
    {
		...
		if(!empty($get['id'])) {
			...
			if (!$galleryParent) {
				...
				$output_xml .= "<gallery>\n";
				$output_xml .=  "	<image targeturl=\"\" target=\"\" path=\"". $GLOBALS['app']->GetJawsURL() . "/gadgets/Ads/images/gallery_no_images.jpg\" textLabel=\"No images have been added or activated for this gallery.\">No images have been added or activated for this gallery.</image>\n";
				$output_xml .= "</gallery>\n";
			} else {
				$image_found = false;
				$output_xml .= "<gallery>\n";
			...
			$output_xml .= "<gallery>\n";
			$output_xml .=  "	<image targeturl=\"\" target=\"\" path=\"". $GLOBALS['app']->GetJawsURL() . "/gadgets/Ads/images/gallery_error.jpg\">Error</image>\n";
			$output_xml .= "</gallery>\n";			
		}
		*/
		return $output_xml;
    ...
    /**
     * Adds a click to ads_clicks db table and redirects to URL.
     *
     * @access public
     * @return string
     */
	    ...
	    if (isset($get['id'])) {
			...
			if (!Jaws_Error::IsError($gallery)) {
				...
				if (Jaws_Error::IsError($result) || !$result) {
					return new Jaws_Error(_t('ADS_ERROR_ADCLICK_NOT_ADDED'), _t('ADS_NAME'));
				} else {
    ...
    /**
     * Displays an individual Ad for embedding.
     *
     * @access public
     * @return string
     */
    ...
    {			
		...
		$tpl->SetVariable('pane_id', str_replace(" ",'',$this->_Name));
		$tpl->SetVariable('pane', 'UserAds');
		$tpl->SetVariable('icon', $GLOBALS['app']->GetJawsURL() . '/gadgets/'.$this->_Name.'/images/logo.png');
						
        $stpl = new Jaws_Template('gadgets/Ads/templates/');
        $stpl->Load('users.html');
        $stpl->SetBlock('UserAdsSubscriptions');
		$status = $jUser->GetStatusOfUserInGroup($GLOBALS['app']->Session->GetAttribute('user_id'), 'ads_owners');
		// Can add?
		$interactive = false;
		if (
			...
			$GLOBALS['app']->ACL->GetFullPermission(
				...
				$GLOBALS['app']->Session->GetAttribute('groups'), 'Ads', 'ManageAds') || 
			$GLOBALS['app']->ACL->GetFullPermission(
				$GLOBALS['app']->Session->GetAttribute('username'), 
				$GLOBALS['app']->Session->GetAttribute('groups'), 'Ads', 'ManagePublicAds') || 
			$GLOBALS['app']->ACL->GetFullPermission(
				$GLOBALS['app']->Session->GetAttribute('username'), 
				$GLOBALS['app']->Session->GetAttribute('groups'), 'Ads', 'default') || 
			in_array($status, array('active','admin','founder'))
		) {
			$interactive = true;
		}
		$usersHTML = $GLOBALS['app']->LoadGadget('Users', 'HTML');
		$page = $usersHTML->ShowComments('Ads', false, null, 'Ads', $interactive);
		if (!Jaws_Error::IsError($page)) {
			$stpl->SetVariable('element', $page);
		} else {
			$stpl->SetVariable('element', _t('GLOBAL_ERROR_GET_ACCOUNT_PANE'));
		}
        $stpl->ParseBlock('UserAdsSubscriptions');
    ...
    {
		//$gadget_admin = $GLOBALS['app']->LoadGadget('Ads', 'AdminHTML');
		return '';
    }
    ...
    function account_form()
    {
		...
		$html_output = $users_html->GetAccountHTML('Ads');
    ...
    function account_form_post()
    {
		...
		$users_html = $GLOBALS['app']->LoadGadget('Users', 'HTML');
		$output_html = str_replace("__JAWS_GADGET__", $page, $users_html->GetAccountHTML('Ads'));
		return $output_html;
    }
    ...
    {
		$gadget_admin = $GLOBALS['app']->LoadGadget('Ads', 'AdminHTML');
		$html_output = str_replace("&nbsp;<input type=\"button\" value=\"Cancel\" onclick=\"if (window.opener && !window.opener.closed) {window.opener.location.reload();window.close();};\">", '', $gadget_admin->SavedAds(true));
		//$output_html = str_replace("<td width=\"100%\" align=\"right\">", "<td width=\"50%\" align=\"right\">", $html_output);
		$output_html = $html_output;
		/*
		$page = $gadget_admin->A(true);
    ...
    function account_ShowEmbedWindow()
    {
		...
		return $user_admin->ShowEmbedWindow('Ads', 'OwnAds', true);
    }
