        ...
        if (response[0]['css'] == 'notice-message') {
            ...
            if (rows.length > 0) {
                for(var i=0; i<rows.length; i++) {
                    $('calendar_datagrid').deleteItem();
                }
            }
            PiwiGrid.multiSelect($('calendar_datagrid'));
            getCalendarData();
        }
        showResponse(response);      
    },
    ...
    savequickadd: function(response) {
		...
		if (response['success']) {
			if (response['addtype'] == 'Scrumy') {
				...
				window.top.saveUpdate(response['id'], comment_html, '', 0, response['sharing'], parent.$('syndication').checked, (parent.$('OwnerID') ? parent.$('OwnerID').value : ''), 'Calendar', true, false, response['eaurl'], false);
			} else {
				window.top.customPageSelectGadget('Calendar', 'AddGadget', '', window.top.prevLinkID, window.top.prevSectionID);
				return true;
...
function saveCalendarReply(cid, id, parent)
{
	...
	response = calendarSync.newcalendarcomment('', comment, cid, id, '', false, 'everyone', (parent != cid ? true : false));
	if (response['css'] == 'notice-message') {
    ...
    if ($('all-comments-'+parent)) {
		$('all-comments-'+parent).innerHTML = '<a href="javascript:void(0);" onclick="toggleCalendarAllComments('+parent+');">View all comments</a>';
	}
	//hideWorkingNotification();
}
...
var calendarAsync = new calendaradminajax(CalendarCallback);
calendarAsync.serverErrorFunc = Jaws_Ajax_ServerError;
calendarAsync.onInit = showWorkingNotification;
calendarAsync.onComplete = hideWorkingNotification;

var calendarSync  = new calendaradminajax();
calendarSync.serverErrorFunc = Jaws_Ajax_ServerError;
calendarSync.onInit = showWorkingNotification;
calendarSync.onComplete = hideWorkingNotification;
