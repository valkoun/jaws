...
Ajax.InPlaceRichEditor = Class.create(Ajax.InPlaceEditor, {
  ...
  initialize: function(element, url, options, tinymce_options) {
    ...
    if (!this.options.externalControl)
      ...
      this.options.highlightColor = 'transparent';
      this.options.highlightEndColor = 'transparent';
    if (!this.options.hoverClassName)
      this.options.hoverClassName = 'inplacericheditor-hover';
  ...
  removeForm: function($super) {
    $(this.element.id).fire('tinymce:onRemove', this.element);
    try {
      this.tinymce.remove();
    } catch(e) {}
	  $super();
